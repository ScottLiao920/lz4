...
#define COMPRESSIBLE_NOISE_LENGTH (2 MB)
    ...
    CHECK_V(cSize,
            LZ4F_compressFrame(compressedBuffer, LZ4F_compressFrameBound(testSize, NULL), CNBuffer, testSize, NULL));
    ...
    DISPLAYLEVEL(3, "LZ4F_compressFrame, using fast level -3 : ");
    {
        ...
        CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer, LZ4F_compressFrameBound(testSize, NULL), CNBuffer, testSize,
                                          &fastCompressPrefs));
    ...
    CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer, LZ4F_compressFrameBound(testSize, &prefs), CNBuffer, testSize,
                                      &prefs));
...
typedef enum {
    o_contiguous, o_noncontiguous, o_overwrite
} o_scenario_e;
...
size_t test_lz4f_decompression(const void *cSrc, size_t cSize,
                               ...
                               U32 seed, U32 testNb,
                               int findErrorPos) {
    o_scenario_e const o_scenario = (o_scenario_e) (FUZ_rand(randState) %
...
int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest, double compressibility, U32 duration_s) {
    ...
    decodedBuffer = calloc(1,
                           CNBufferLength);   /* calloc avoids decodedBuffer being considered "garbage" by scan-build */
    ...
    for (; (testNb < nbTests) || (clockDuration > FUZ_GetClockSpan(startClock)); testNb++) {
        ...
        if ((FUZ_rand(&randState) & 0xFFF) == 0) {
            ...
            cSize = LZ4F_compressFrame(compressedBuffer, LZ4F_compressFrameBound(srcSize, prefsPtr), srcStart, srcSize,
                                       prefsPtr);
            ...
            BYTE *op = (BYTE *) compressedBuffer;
            BYTE *const oend = op + (neverFlush ? LZ4F_compressFrameBound(srcSize, prefsPtr)
        ...
        {
            size_t result = test_lz4f_decompression(compressedBuffer, cSize, srcStart, srcSize, crcOrig, &randState,
                                                    dCtx, seed, testNb, 1 /*findError*/ );
        ...
        test_lz4f_decompression(compressedBuffer, cSize, srcStart, srcSize, crcOrig, &randState, dCtxNoise, seed,
