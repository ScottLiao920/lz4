...
static int FUZ_AddressOverflow(void) {
    ...
    for (nbBuff = 2; nbBuff < MAX_NB_BUFF_I134; nbBuff++) {
...

        {
            ...
            {
                int u;
                for (u = 4; u <= nbOf255 + 4; u++) input[u] = (char) 0xff;
            }
            r = LZ4_decompress_safe(input, output, nbOf255 + 64, BLOCKSIZE_I134);
            if (r > 0) {
                DISPLAY("LZ4_decompress_safe = %i \n", r);
                goto _overflowError;
            }
    ...
    if (!highAddress) DISPLAY("high address not possible \n");
        ...
        DISPLAY("all overflows correctly detected \n");
    return 0;
...
#   define FUZ_DISPLAYTEST(...) {                 \
    ...
    for (cycleNb = startCycle;
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_HC_destSize()");
        {
            ...
            FUZ_CHECKTEST(compressedBuffer[targetSize] != endCheck,
            ...
            if (targetSize > 0) {
                ...
                decodedBuffer[srcSize] = canary;
                {
                    int const dSize = LZ4_decompress_safe(compressedBuffer, decodedBuffer, cSize, srcSize);
                    FUZ_CHECKTEST(dSize < 0,
                                  "LZ4_decompress_safe failed (%i) on data compressed by LZ4_compressHC_destSize",
                                  dSize);
                    FUZ_CHECKTEST(dSize != srcSize,
                                  "LZ4_decompress_safe failed : decompressed %i bytes, was supposed to decompress %i bytes",
                                  dSize, srcSize);
                }
                FUZ_CHECKTEST(decodedBuffer[srcSize] != canary, "LZ4_decompress_safe overwrite dst buffer !");
                {
        ...
        HCcompressedSize = LZ4_compress_HC(block, compressedBuffer, blockSize, (int) compressedBufferSize,
                                           compressionLevel);
        ...
        {
            int const r = LZ4_compress_HC_extStateHC(stateLZ4HC, block, compressedBuffer, blockSize,
                                                     (int) compressedBufferSize, compressionLevel);
        ...
        {
            int const r = LZ4_compress_HC_extStateHC_fastReset(stateLZ4HC, block, compressedBuffer, blockSize,
                                                               (int) compressedBufferSize, compressionLevel);
        ...
        {
            int const r = LZ4_compress_fast_extState(stateLZ4, block, compressedBuffer, blockSize,
                                                     (int) compressedBufferSize, 8);
        ...
        FUZ_DISPLAYTEST();
        {
            int const r = LZ4_compress_fast_extState_fastReset(stateLZ4, block, compressedBuffer, blockSize,
                                                               (int) compressedBufferSize, 8);
        ...
        /* Test decompress_fast() with input buffer size exactly correct => must not read out of bound */
        {
            ...
            if (blockSize > 10) {
                ...
                FUZ_CHECKTEST(decodedBuffer[blockSize - 10],
            ...
            FUZ_DISPLAYTEST("decompress noisy source ");
            {
                ...
                {
                    int const decompressResult = LZ4_decompress_safe(cBuffer_exact, decodedBuffer, compressedSize,
                                                                     blockSize);
                    /* result *may* be an unlikely success, but even then, it must strictly respect dst buffer boundaries */
                    FUZ_CHECKTEST(decompressResult > blockSize,
                                  "LZ4_decompress_safe on noisy src : result is too large : %u > %u (dst buffer)",
                                  (unsigned) decompressResult, (unsigned) blockSize);
                }
                {
                    U32 endCheck;
                    memcpy(&endCheck, decodedBuffer + blockSize, sizeof(endCheck));
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            FUZ_CHECKTEST(r >= 0,
                          "LZ4_decompress_safe should have failed, due to input size being one byte too short (blockSize=%i, result=%i, compressedSize=%i)",
                          blockSize, r, compressedSize);
        }
        ...
        FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial");
        {
            ...
            int const decResult = LZ4_decompress_safe_partial(compressedBuffer, decodedBuffer, inCSize, targetSize,
                                                              blockSize);
            FUZ_CHECKTEST(decResult < 0, "LZ4_decompress_safe_partial failed despite valid input data (error:%i)",
                          decResult);
            FUZ_CHECKTEST(decResult != targetSize,
                          "LZ4_decompress_safe_partial did not regenerated required amount of data (%i < %i <= %i)",
                          decResult, targetSize, blockSize);
            FUZ_CHECKTEST(decodedBuffer[targetSize] != sentinel,
                          "LZ4_decompress_safe_partial overwrite beyond requested size (though %i <= %i <= %i)",
                          decResult, targetSize, blockSize);
            FUZ_CHECKTEST(memcmp(block, decodedBuffer, (size_t) targetSize),
        ...
        ret = LZ4_compress_HC_extStateHC(stateLZ4HC, block, compressedBuffer, blockSize, HCcompressedSize,
                                         compressionLevel);
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_default() with output buffer a bit too short");
        {
            ...
            {
                int const cSize = LZ4_compress_default(block, compressedBuffer, blockSize,
                                                       compressedSize - missingBytes);
                FUZ_CHECKTEST(cSize, "LZ4_compress_default should have failed (output buffer too small by %i byte)",
                              missingBytes);
            }
            FUZ_CHECKTEST(compressedBuffer[compressedSize - missingBytes],
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_HC() with output buffer a bit too short");
        {
            ...
            {
                int const hcSize = LZ4_compress_HC(block, compressedBuffer, blockSize, HCcompressedSize - missingBytes,
                                                   compressionLevel);
                FUZ_CHECKTEST(hcSize, "LZ4_compress_HC should have failed (output buffer too small by %i byte)",
                              missingBytes);
            }
            FUZ_CHECKTEST(compressedBuffer[HCcompressedSize - missingBytes],
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_fast_continue() with dictionary of size %i", dictSize);
        {
            ...
            LZ4_compress_fast_continue(&LZ4_stream, dict, compressedBuffer, dictSize, (int) compressedBufferSize,
                                       1);   /* Just to fill hash tables */
            blockContinueCompressedSize = LZ4_compress_fast_continue(&LZ4_stream, block, compressedBuffer, blockSize,
        ...
        ret = LZ4_decompress_fast_usingDict(compressedBuffer, decodedBuffer + dictSize, blockSize, decodedBuffer,
                                            dictSize);
        FUZ_CHECKTEST(ret != blockContinueCompressedSize,
        ...
        ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer + dictSize, blockContinueCompressedSize,
                                            blockSize, decodedBuffer, dictSize);
        ...
        blockContinueCompressedSize = LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer, blockSize,
                                                                 (int) compressedBufferSize, 1);
        FUZ_CHECKTEST(blockContinueCompressedSize == 0, "LZ4_compress_fast_continue failed");

        FUZ_DISPLAYTEST("LZ4_compress_fast_continue() with dictionary and output buffer too short by one byte");
        LZ4_loadDict(&LZ4dictBody, dict, dictSize);
        ret = LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer, blockSize,
                                         blockContinueCompressedSize - 1, 1);
        FUZ_CHECKTEST(ret > 0,
                      "LZ4_compress_fast_continue using ExtDict should fail : one missing byte for output buffer : %i written, %i buffer",
        ...
        LZ4_loadDict(&LZ4dictBody, dict, dictSize);
        ret = LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer, blockSize, blockContinueCompressedSize,
                                         1);
        FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                      "LZ4_compress_limitedOutput_compressed size is different (%i != %i)", ret,
                      blockContinueCompressedSize);
        ...
        FUZ_DISPLAYTEST();
        decodedBuffer[blockSize] = 0;
        ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer, blockContinueCompressedSize, blockSize,
                                            dict, dictSize);
        FUZ_CHECKTEST(ret != blockSize, "LZ4_decompress_safe_usingDict did not regenerate original data");
        ...
        FUZ_CHECKTEST(decodedBuffer[blockSize - 1],
                      "LZ4_decompress_fast_usingDict overrun specified output buffer size");

        FUZ_DISPLAYTEST();
        decodedBuffer[blockSize - 1] = 0;
        ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer, blockContinueCompressedSize, blockSize - 1,
                                            dict, dictSize);
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            if (blockSize > missingBytes) {
                ...
                ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer, blockContinueCompressedSize,
                                                    blockSize - missingBytes, dict, dictSize);
                FUZ_CHECKTEST(ret >= 0,
                              "LZ4_decompress_safe_usingDict should have failed : output buffer too small (-%i byte)",
                              missingBytes);
                FUZ_CHECKTEST(decodedBuffer[blockSize - missingBytes],
                              "LZ4_decompress_safe_usingDict overrun specified output buffer size (-%i byte) (blockSize=%i)",
        ...
        /* Compress using external dictionary stream */
        {
            ...
            LZ4_loadDict(&LZ4dictBody, dict, dictSize);
            expectedSize = LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer, blockSize,
                                                      (int) compressedBufferSize, 1);
            FUZ_CHECKTEST(expectedSize <= 0,
             ...
             * test.
             */
            FUZ_CHECKTEST(blockContinueCompressedSize != expectedSize,
                          "LZ4_compress_fast_continue using extDictCtx produced different-sized output (%d expected vs %d actual)",
                          expectedSize, blockContinueCompressedSize);
            FUZ_CHECKTEST(XXH32(compressedBuffer, (size_t) blockContinueCompressedSize, 0) != expectedCrc,
            ...
            LZ4_attach_dictionary(&LZ4_stream, &LZ4dictBody);
            ret = LZ4_compress_fast_continue(&LZ4_stream, block, compressedBuffer, blockSize,
                                             blockContinueCompressedSize - 1, 1);
            FUZ_CHECKTEST(ret > 0,
                          "LZ4_compress_fast_continue using extDictCtx should fail : one missing byte for output buffer : %i written, %i buffer",
                          ret, blockContinueCompressedSize);
        ...
        blockContinueCompressedSize = LZ4_compress_HC_continue(LZ4dictHC, block, compressedBuffer, blockSize,
        ...
        FUZ_CHECKTEST(ret > 0,
                      "LZ4_compress_HC_continue using ExtDict should fail : one missing byte for output buffer (expected %i, but result=%i)",
                      blockContinueCompressedSize, ret);
        /* note : context is no longer dirty after a failed compressed block */
        ...
        FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                      "LZ4_compress_HC_continue size is different : ret(%i) != expected(%i)", ret,
                      blockContinueCompressedSize);
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            blockContinueCompressedSize = LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize,
                                                                   (int) compressedBufferSize);
            ...
            LZ4_attach_HC_dictionary(LZ4_streamHC, LZ4dictHC);
            ret = LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize,
                                           blockContinueCompressedSize - 1);
            FUZ_CHECKTEST(ret > 0,
                          "LZ4_compress_HC_continue using ExtDictCtx should fail : one missing byte for output buffer (%i != %i)",
                          ret, blockContinueCompressedSize);
            /* note : context is no longer dirty after a failed compressed block */
            ...
            ret = LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize,
                                           blockContinueCompressedSize);
            FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                          "LZ4_compress_HC_continue using ExtDictCtx size is different (%i != %i)", ret,
                          blockContinueCompressedSize);
            FUZ_CHECKTEST(ret <= 0,
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            LZ4_loadDictHC(LZ4dictHC, dict, dictSize);
            LZ4_setCompressionLevel(LZ4dictHC, compressionLevel);
            blockContinueCompressedSize = LZ4_compress_HC_continue_destSize(LZ4dictHC, block, compressedBuffer,
                                                                            &consumedSize, availableSpace);
            ...
            FUZ_CHECKTEST(blockContinueCompressedSize > availableSpace,
            ...
            ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer, blockContinueCompressedSize,
                                                consumedSize, dict, dictSize);
            FUZ_CHECKTEST(ret != consumedSize, "LZ4_decompress_safe_usingDict regenerated %i bytes (%i expected)", ret,
                          consumedSize);
            FUZ_CHECKTEST(decodedBuffer[consumedSize],
                          "LZ4_decompress_safe_usingDict overrun specified output buffer size")
            {
...
static void FUZ_unitTests(int compressionLevel) {
    ...
    DISPLAYLEVEL(3, "LZ4_initStream with multiple valid alignments : ");
    {
        ...
        DISPLAYLEVEL(4, "state1(%p) state2(%p) state3(%p) LZ4_stream_t size(0x%x): ",
                     (void *) &(shc->state1), (void *) &(shc->state2), (void *) &(shc->state3),
                     (unsigned) sizeof(LZ4_stream_t));
        FUZ_CHECKTEST(LZ4_initStream(&(shc->state1), sizeof(shc->state1)) == NULL, "state1 (%p) failed init",
                      (void *) &(shc->state1));
        FUZ_CHECKTEST(LZ4_initStream(&(shc->state2), sizeof(shc->state2)) == NULL, "state2 (%p) failed init",
                      (void *) &(shc->state2));
        FUZ_CHECKTEST(LZ4_initStream(&(shc->state3), sizeof(shc->state3)) == NULL, "state3 (%p) failed init",
                      (void *) &(shc->state3));
    ...
    /* LZ4 streaming tests */
    {
        ...
        {
            int const cs = LZ4_compress_fast_continue(&streamingState, testInput, testCompressed, testCompressedSize,
                                                      testCompressedSize - 1, 1);
        ...
        /* ring buffer test */
        {
            ...
            while (iNext + messageSize < testCompressedSize) {
                ...
                compressedSize = LZ4_compress_fast_continue(&streamingState, ringBuffer + rNext, testCompressed,
                                                            (int) messageSize, testCompressedSize - ringBufferSize, 1);
                FUZ_CHECKTEST(compressedSize == 0, "LZ4_compress_fast_continue() compression failed");

                {
                    int const r = LZ4_decompress_safe_continue(&decodeStateSafe, testCompressed, testVerify + dNext,
                                                               compressedSize, (int) messageSize);
                ...
                {
                    int const r = LZ4_decompress_fast_continue(&decodeStateFast, testCompressed, testVerify + dNext,
                                                               (int) messageSize);
    ...
    DISPLAYLEVEL(3, "LZ4_initStreamHC with multiple valid alignments : ");
    {
        ...
        FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc1), sizeof(shc->hc1)) == NULL, "hc1 (%p) failed init",
                      (void *) &(shc->hc1));
        FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc2), sizeof(shc->hc2)) == NULL, "hc2 (%p) failed init",
                      (void *) &(shc->hc2));
        FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc3), sizeof(shc->hc3)) == NULL, "hc3 (%p) failed init",
                      (void *) &(shc->hc3));
    ...
    /* LZ4 HC streaming tests */
    {
        ...
        DISPLAYLEVEL(3, "Simple HC round-trip : ");
        {
            ...
            result = LZ4_compress_HC_continue(&sHC, testInput, testCompressed, testCompressedSize,
                                              testCompressedSize - 1);
        ...
        DISPLAYLEVEL(3, "Long sequence HC_destSize test : ");
        {
            ...
            result = LZ4_compress_HC_destSize(&sHC, (const char *) block, (char *) dstBlock, &srcSize, (int) targetSize,
                                              3);
        ...
        DISPLAYLEVEL(3, "HC dictionary compression test : ");
        {
            ...
            {
                int const cSize = LZ4_compress_HC_continue(&sHC, testInput + 64 KB, testCompressed, testCompressedSize,
                                                           testCompressedSize - 1);
                FUZ_CHECKTEST(cSize == 0,
                              "LZ4_compressHC_limitedOutput_continue() dictionary compression failed : @return = %i",
                              cSize);
                FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
                {
                    int const dSize = LZ4_decompress_safe_usingDict(testCompressed, testVerify, cSize,
                                                                    testCompressedSize, testInput, 64 KB);
                    FUZ_CHECKTEST(dSize != (int) testCompressedSize,
                                  "LZ4_decompress_safe() simple dictionary decompression test failed");
                }
            }
            {
                U64 const crcNew = XXH64(testVerify, testCompressedSize, 0);
                FUZ_CHECKTEST(crc64 != crcNew,
        ...
        /* multiple HC compression test with dictionary */
        {
            ...
            FUZ_CHECKTEST(result1 == 0,
                          "LZ4_compressHC_limitedOutput_continue() dictionary compression failed : result = %i",
                          result1);
            FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
            result2 = LZ4_compress_HC_continue(&sHC, testInput + 2 * (size_t) segSize, testCompressed + result1,
                                               segSize, segSize - 1);
            FUZ_CHECKTEST(result2 == 0,
                          "LZ4_compressHC_limitedOutput_continue() dictionary compression failed : result = %i",
                          result2);
            FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
            ...
            result = LZ4_decompress_safe_usingDict(testCompressed + result1, testVerify + segSize, result2, segSize,
                                                   testInput, 2 * segSize);
        ...
        /* remote dictionary HC compression test */
        {
            ...
            result = LZ4_compress_HC_continue(&sHC, testInput + 64 KB, testCompressed, testCompressedSize,
                                              testCompressedSize - 1);
            FUZ_CHECKTEST(result == 0, "LZ4_compressHC_limitedOutput_continue() remote dictionary failed : result = %i",
                          result);
            FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

            result = LZ4_decompress_safe_usingDict(testCompressed, testVerify, result, testCompressedSize, testInput,
                                                   32 KB);
            FUZ_CHECKTEST(result != (int) testCompressedSize,
        ...
        /* multiple HC compression with ext. dictionary */
        {
            ...
            while (segStart + segSize < testInputSize) {
                ...
                result = LZ4_compress_HC_continue(&sHC, testInput + segStart, testCompressed, (int) segSize,
                                                  LZ4_compressBound((int) segSize));
                FUZ_CHECKTEST(result == 0,
                              "LZ4_compressHC_limitedOutput_continue() dictionary compression failed : result = %i",
                              result);
                FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

                result = LZ4_decompress_safe_usingDict(testCompressed, dst, result, (int) segSize, dict,
                                                       (int) dictSize);
                FUZ_CHECKTEST(result != (int) segSize,
                              "LZ4_decompress_safe_usingDict() dictionary decompression part %i failed", (int) segNb);
        ...
        /* ring buffer test */
        {
            ...
            while (iNext + messageSize < testCompressedSize) {
                ...
                compressedSize = LZ4_compress_HC_continue(&sHC, ringBuffer + rNext, testCompressed, (int) messageSize,
                                                          testCompressedSize - ringBufferSize);
                FUZ_CHECKTEST(compressedSize == 0, "LZ4_compress_HC_continue() compression failed");
                FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

                assert(messageSize < INT_MAX);
                result = LZ4_decompress_safe_continue(&decodeStateSafe, testCompressed, testVerify + dNext,
                                                      compressedSize, (int) messageSize);
                ...
                assert(messageSize < INT_MAX);
                result = LZ4_decompress_fast_continue(&decodeStateFast, testCompressed, testVerify + dNext,
                                                      (int) messageSize);
         ...
         */
        {
            ...
            assert((size_t) dBufferSize * 2 + 1 <
                   testInputSize);   /* space used by ringBufferSafe and ringBufferFast */
            XXH64_reset(&xxhOrig, 0);
            XXH64_reset(&xxhNewSafe, 0);
            XXH64_reset(&xxhNewFast, 0);
            LZ4_resetStreamHC_fast(&sHC, compressionLevel);
...
#define BSIZE1 (dBufferSize - (maxMessageSize-1))
            ...
            compressedSize = LZ4_compress_HC_continue(&sHC, testInput + iNext, testCompressed, messageSize,
                                                      testCompressedSize - ringBufferSize);
            FUZ_CHECKTEST(compressedSize == 0, "LZ4_compress_HC_continue() compression failed");
            FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

            result = LZ4_decompress_safe_continue(&decodeStateSafe, testCompressed, ringBufferSafe + dNext,
            ...
            result = LZ4_decompress_fast_continue(&decodeStateFast, testCompressed, ringBufferFast + dNext,
                                                  messageSize);
            ...
            iNext = BSIZE1 + 1;
            assert(BSIZE1 >= 65535);
            memcpy(testInput + iNext, testInput + (BSIZE1 - 65535),
                   messageSize);  /* will generate a match at max distance == 65535 */
            FUZ_CHECKTEST(dNext + messageSize <= dBufferSize,
            ...
            while (totalMessageSize < 9 MB) {
                ...
                result = LZ4_decompress_safe_continue(&decodeStateSafe,
                                                      ...
                                                      compressedSize, dBufferSize -
                                                                      dNext);   /* works without knowing messageSize, under assumption that messageSize <= maxMessageSize */
    ...
    /* encode congenerical sequence test for HC compressors */
    {
        ...
        for (dlen = dst_min_len; dlen <= dst_max_len; dlen += dst_step) {
            ...
            for (slen = src_len - 3; slen <= src_len + 3; slen++) {
                ...
                FUZ_CHECKTEST(memcmp(dbuf2, dbuf1, (size_t) dsz2),
                ...
                FUZ_CHECKTEST(memcmp(sbuf1, sbuf2, (size_t) res2),
