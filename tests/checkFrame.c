    ...
    for (;;) {
        ...
        while (pos < readSize) {  /* still to read */
            ...
            if (!nextToLoad) {
                ...
                if (LZ4F_isError(nextToLoad)) EXM_THROW(22, "Error getting frame info: %s",
                                                        LZ4F_getErrorName(nextToLoad));
                if (frameInfo.blockSizeID != (LZ4F_blockSizeID_t) bsid) EXM_THROW(23, "Block size ID %u != expected %u",
            ...
            if (!partialBlock) {
                ...
                if ((curblocksize != 0) && (nextToLoad > 4)) {
                    if (curblocksize != blockSize) EXM_THROW(25, "Block size %u != expected %u, pos %u\n",
                                                             (unsigned) curblocksize, (unsigned) blockSize,
                                                             (unsigned) pos);
                }
                curblocksize = 0;
            }
