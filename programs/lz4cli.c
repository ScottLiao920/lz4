...
int LZ4IO_compressFilename_Legacy(const char *input_filename, const char *output_filename, int compressionlevel,
                                  const LZ4IO_prefs_t *prefs);   /* hidden function */
...
typedef enum {
    om_auto, om_compress, om_decompress, om_test, om_bench, om_list
} operationMode_e;
...
int main(int argc, const char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!all_arguments_are_files && argument[0] == '-') {
            ...
            if (argument[1] == '-') {
                ...
                if (!strcmp(argument, "--no-force")) {
                    LZ4IO_setOverwrite(prefs, 0);
                    continue;
                }
                if ((!strcmp(argument, "--stdout"))
                    || (!strcmp(argument, "--to-stdout"))) {
                    forceStdout = 1;
                    output_filename = stdoutmark;
                    continue;
                }
                if (!strcmp(argument, "--frame-crc")) {
                    LZ4IO_setStreamChecksumMode(prefs, 1);
                    continue;
                }
                if (!strcmp(argument, "--no-frame-crc")) {
                    LZ4IO_setStreamChecksumMode(prefs, 0);
                    continue;
                }
                if (!strcmp(argument, "--content-size")) {
                    LZ4IO_setContentSize(prefs, 1);
                    continue;
                }
                if (!strcmp(argument, "--no-content-size")) {
                    LZ4IO_setContentSize(prefs, 0);
                    continue;
                }
                ...
                if (!strcmp(argument, "--sparse")) {
                    LZ4IO_setSparseFile(prefs, 2);
                    continue;
                }
                if (!strcmp(argument, "--no-sparse")) {
                    LZ4IO_setSparseFile(prefs, 0);
                    continue;
                }
                if (!strcmp(argument, "--favor-decSpeed")) {
                    LZ4IO_favorDecSpeed(prefs, 1);
                    continue;
                }
                ...
                if (!strcmp(argument, "--rm")) {
                    LZ4IO_setRemoveSrcFile(prefs, 1);
                    continue;
                }
            ...
            while (argument[1] != 0) {
                ...
                if (g_lz4c_legacy_commands) {
                    ...
                    if (!strcmp(argument, "c0")) {
                        cLevel = 0;
                        argument++;
                        continue;
                    }  /* -c0 (fast compression) */
                    if (!strcmp(argument, "c1")) {
                        cLevel = 9;
                        argument++;
                        continue;
                    }  /* -c1 (high compression) */
                    if (!strcmp(argument, "c2")) {
                        cLevel = 12;
                        argument++;
                        continue;
                    } /* -c2 (very high compression) */
                    if (!strcmp(argument, "hc")) {
                        cLevel = 12;
                        argument++;
                        continue;
                    } /* -hc (very high compression) */
                    if (!strcmp(argument, "y")) {
                        LZ4IO_setOverwrite(prefs, 1);
                        continue;
                ...
                switch (argument[0]) {
                    ...
                    case 'B':
                        while (argument[1] != 0) {
                            ...
                            switch (argument[1]) {
                                case 'D':
                                    LZ4IO_setBlockMode(prefs, LZ4IO_blockLinked);
                                    argument++;
                                    break;
                                case 'I':
                                    LZ4IO_setBlockMode(prefs, LZ4IO_blockIndependent);
                                    argument++;
                                    break;
                                case 'X':
                                    LZ4IO_setBlockChecksumMode(prefs, 1);
                                    argument++;
    ...
    while ((!output_filename) && (multiple_inputs == 0)) {
        ...
        if (mode ==
            om_decompress) {/* decompression to file (automatic name will work only if input filename has correct format extension) */
            size_t outl;
            ...
            outl = inl;
            if (inl > 4)
                while ((outl >= inl - 4) && (input_filename[outl] == extension[outl - inl + 4]))
                    dynNameSpace[outl--] = 0;
            if (outl != inl - 5) {
                DISPLAYLEVEL(1, "Cannot determine an output filename\n");
                badusage(exeName);
            }
            output_filename = dynNameSpace;
    ...
    if (mode == om_decompress) {
        ...
        if (legacy_format) {
            ...
            if (multiple_inputs) {
                ...
                assert(ifnIdx <= INT_MAX);
                operationResult = LZ4IO_compressMultipleFilenames(inFileNames, (int) ifnIdx, comp_extension, cLevel,
                                                                  prefs);
            }
            else {
