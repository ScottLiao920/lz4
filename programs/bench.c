...
static int BMK_benchMem(const void *srcBuffer, size_t srcSize,
    ...
    size_t const maxCompressedSize =
            LZ4_compressBound((int) srcSize) + (maxNbBlocks * 1024);   /* add some room for safety */
    ...
    /* checks */
    if (!compressedBuffer || !resultBuffer || !blockTable) EXM_THROW(31, "allocation error : not enough memory");
    ...
    /* Bench */
    {
        ...
        U32 nbCompressionLoops =
                (U32) ((5 MB) / (srcSize + 1)) + 1;  /* conservative initial compression speed estimate */
...
#       define NB_MARKS 4
        ...
        if (g_displayLevel == 1) {
            ...
            if (g_additionalParam)
                DISPLAY("-%-3i%11i (%5.3f) %6.2f MB/s %6.1f MB/s  %s (param=%d)\n", cLevel, (int) cSize, ratio, cSpeed,
                        dSpeed, displayName, g_additionalParam);
            else
...
static void BMK_benchCLevel(void *srcBuffer, size_t benchedSize,
    ...
    if (g_displayLevel == 1 && !g_additionalParam)
        DISPLAY("bench %s %s: input %u bytes, %u seconds, %u KB blocks\n", LZ4_VERSION_STRING, LZ4_GIT_COMMIT_STRING,
                (U32) benchedSize, g_nbSeconds, (U32) (g_blockSize >> 10));
...
int BMK_benchFiles(const char **fileNamesTable, unsigned nbFiles,
    ...
    if (dictFileName) {
        ...
        if (dictFileSize > LZ4_MAX_DICT_SIZE) {
            ...
            if (UTIL_fseek(dictFile, dictFileSize - dictSize, SEEK_SET)) EXM_THROW(25,
        ...
        if (fread(dictBuf, 1, dictSize, dictFile) != (size_t) dictSize) EXM_THROW(25,
